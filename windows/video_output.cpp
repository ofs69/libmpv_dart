#include "libmpv_dart/video_output.h"
...
VideoOutput::VideoOutput(int64_t handle, VideoOutputConfiguration configuration,
  ...
  auto future = thread_pool_ref_->Post([&]() {
    ...
    if (configuration.enable_hardware_acceleration) {
      try {
        ...
        if (mpv_render_context_create(&render_context_, handle_, params) == 0) {
          ...
          // std::cout << "libmpv_dart: VideoOutput: Using H/W rendering."
          //           << std::endl;
        }
      } catch (...) {
        // Do nothing.
    ...
    if (!is_hardware_acceleration_enabled) {
      // std::cout << "libmpv_dart: VideoOutput: Using S/W rendering." <<
      // std::endl; Allocate a "large enough" buffer ahead of time.
      pixel_buffer_ =
    ...
    registrar_->texture_registrar()->UnregisterTexture(
        texture_id_, [&, texture_id = texture_id_]() {
          ...
          auto future = thread_pool_ref_->Post([&, id = texture_id]() {
            // std::cout << "libmpv_dart: VideoOutput: Free Texture: " << id
            //           << std::endl;
...
int64_t VideoOutput::SetSize(std::optional<int64_t> width,
                          std::optional<int64_t> height) {
  auto promise = std::promise<void>();
  auto future = thread_pool_ref_->Post([&, width, height]() {
    if (width.has_value()) {
      // H/W
      if (surface_manager_ != nullptr) {
        width_ = width.value();
  ...
  // Wait for the new frame rendered to get tht new texture id
  promise.get_future().wait();
  return texture_id_;
}
...
void VideoOutput::Resize(int64_t required_width, int64_t required_height) {
  ...
  if (surface_manager_ != nullptr) {
    ...
    auto texture_variant =
        std::make_unique<flutter::TextureVariant>(flutter::GpuSurfaceTexture(
            kFlutterDesktopGpuSurfaceTypeDxgiSharedHandle, [&](auto, auto) {
              ...
              // It crashes when using default mpv properties
              // Since the function may return nullptr, why do we need to use .at?
              // TODO: I will refactor all of these in the future
              // 
              // if (texture_id_) {
    ...
    // std::cout << "libmpv_dart: VideoOutput: Create Texture: " << texture_id_
    //           << std::endl;
    std::lock_guard<std::mutex> lock(textures_mutex_);
